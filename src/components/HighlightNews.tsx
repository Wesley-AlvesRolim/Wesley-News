...
export function HighlightNews({ news }) {
  ...
  const handleHighlightNewsClick = useCallback(() => {
    router.push(`/news/${news.sys.id}`);
  }, []);
  return (
    <div
      className="w-4/5 m-auto mt-[-42px] p-6 rounded-tl-[1.875rem]  bg-white flex flex-wrap-reverse justify-center md:p-14 xl:flex-nowrap hover:cursor-pointer"
      onClick={handleHighlightNewsClick}
    >
